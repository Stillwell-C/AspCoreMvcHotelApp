namespace WebApp.Models
{
    public class HotelsRepository
    {
        ...
        public static List<Hotel> GetHotels() => _hotels;

        public static Hotel? GetHotel(int id)
        {
            return _hotels.FirstOrDefault(x => x.Hotel_id == id) ?? null;
        }

        public static List<Hotel> SearchHotels(string searchTerm, int? priceMin, int? priceMax)
        {
            var priceMinFilter = priceMin.HasValue ? priceMin.Value : 0;
            var priceMaxFilter = priceMax.HasValue ? priceMax.Value : int.MaxValue;
            var lowerSearchTerm = searchTerm.ToLower();
            return _hotels.Where(x =>
                ...
                .Where(x => x.Rates_from >= priceMinFilter)
                .Where(x => x.Rates_from <= priceMaxFilter)
                .ToList();
        }

        public static List<Hotel> GetPopularHotels()
        {
            return _hotels.OrderByDescending(x => x.Rating_average).Take(4).ToList();
        }

        public static List<Hotel> GetFiveStarHotels()
        {
            return _hotels.Where(x => x.Star_rating == 5).Take(4).ToList();
        }

        public static List<Hotel> GetDealHotels()
        {
            return _hotels.OrderBy(x => x.Rates_from).Take(4).ToList();
        }

        public static List<HotelsByContinent> GetHotelsContinentCount()
        {
            var europeCount = new HotelsByContinent { Name = "Europe", SearchTerm = "europe", Count = _hotels.Where(x => x.Continent_name.ToLower() == "europe").Count(), img = _hotels.Where(x => x.Continent_name.ToLower() == "europe").First().Photo1};
            var asiaCount = new HotelsByContinent { Name = "Asia", SearchTerm = "asia", Count = _hotels.Where(x => x.Continent_name.ToLower() == "asia").Count(), img = _hotels.Where(x => x.Continent_name.ToLower() == "asia").First().Photo1 };
            var americasCount = new HotelsByContinent { Name = "Americas", SearchTerm = "america", Count = _hotels.Where(x => x.Continent_name.ToLower().Contains("america")).Count(), img = _hotels.Where(x => x.Continent_name.ToLower().Contains("america")).First().Photo1 };
/*            var africaCount = new HotelsByContinent { Name = "Africa", Count = _hotels.Where(x => x.Continent_name.ToLower() == "africa").Count(), img = _hotels.Where(x => x.Continent_name.ToLower() == "africa").First().Photo1 };
*/
